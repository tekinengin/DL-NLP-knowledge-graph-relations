...
# We project onto 46 unit output layer, and squash it with a sigmoid:
model.add(Dense(46))
...
for i in y_pred:
    ...
    for index,p in enumerate(i):
        if(p>0.5 and data.columns[index+2] != 'NO_REL'):    
...
df.to_csv('hw1_pred_CNN.csv',index=False)
